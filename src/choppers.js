...
export class Chopper {
  ...
  update(dt) {
    ...
    // ---- chase an orbit point ~14u off the player; the angle drifts so it circles
    this.orbitAngle += this.orbitDrift * dt;
    const px = player ? player.pos.x : this.pos.x;
    const pz = player ? player.pos.z : this.pos.z;
    const tx = px + Math.cos(this.orbitAngle) * ORBIT_R;
    const tz = pz + Math.sin(this.orbitAngle) * ORBIT_R;
