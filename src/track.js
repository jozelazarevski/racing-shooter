...
export class Track {
  ...
  _planOverpasses() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (let j = i + 40; j < N; j++) {
        // FIX (Kimi, 2026-08-16): replaced asymmetric i<40&&j>N-40 check
        // with proper circular distance. The old check missed wrap-around
        // neighbours (e.g. i=N-5, j=45), causing false overpass detections
        // at lap boundaries and missed crossings near index 0.
        const circDist = Math.min(Math.abs(i - j), N - Math.abs(i - j));
        if (circDist < 40) continue;                  // circular neighbours
  ...
  terrainHeight(x, z) {
    ...
    const ns = this._nearestSample(x, z);
    let nd = ns.d, bi = ns.i;
    // FIX (Kimi, 2026-08-16): _nearestSample is XZ-only, so at an overpass
    // crossing it can pick the elevated leg when the query point is on the
    // lower leg. The terrain then pokes through the lower road (car clips
    // into a hillside that is actually the upper leg's height blended down).
    // If bi landed on the wrong leg, switch to the correct one.
    if (this._overpasses?.length && nd < 22) {
      for (const o of this._overpasses) {
        const nearUp = this._circDist(bi, o.up) <= o.half + 5;
        const nearDown = this._circDist(bi, o.down) <= o.half + 5;
        if (!nearUp && !nearDown) continue;
        const latUp = Math.abs((x - this.center[o.up].x) * this.nrm[o.up].x
                             + (z - this.center[o.up].z) * this.nrm[o.up].z);
        const latDown = Math.abs((x - this.center[o.down].x) * this.nrm[o.down].x
                               + (z - this.center[o.down].z) * this.nrm[o.down].z);
        const w = (this.widthAt ? this.widthAt(o.up) : 9) + 2;
        const onUp = latUp < w, onDown = latDown < w;
        if (nearUp && onDown && !onUp) {
          bi = o.down;
          nd = Math.hypot(x - this.center[bi].x, z - this.center[bi].z);
          break;
        } else if (nearDown && onUp && !onDown) {
          bi = o.up;
          nd = Math.hypot(x - this.center[bi].x, z - this.center[bi].z);
          break;
        }
