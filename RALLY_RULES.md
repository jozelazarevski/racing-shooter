...
| HRD-1a | AMENDMENT to HRD-1, on the E-9 hazard frame (2026-09-10) and the owner's "Yes" to literal blockers | ONE named class may sit inside the drivable width, and nothing else. A CHARGE PLATE: lies flat on the surface, total height <= 0.25 m so no wheel is lifted and no car is stopped by it; is DESTRUCTIBLE — it detonates on contact or under fire and is gone; never applies a velocity stop or a hard collision response, only its blast; is telegraphed at least 60 m ahead on the racing line (readable at the world's design speed, on every palette, per §7.9); never inside a gate clear zone, an apex band, a kicker landing zone, or the finish run-out; at most one per 150 m of road. Everything HRD-1 forbade still stands for every other class — a crate, a barrel, a rock, a fence, a sign or a plant in the carriageway remains a hard-gate failure. `test-nothing-on-road` gains this one class as a typed allowance with those bounds asserted, NOT a by-name pin and NOT a waiver. | SPEC'D, NOT BUILT |
