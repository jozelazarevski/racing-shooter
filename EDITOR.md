...
| Tool | What it does |
|---|---|
| **PLACE** | Drop the selected preset from the palette. Ghost markers show the layout; the real buildings appear at APPLY. The palette carries houses, cottages, the Riviera terraces, working buildings (barn, silo, windmill, water tower, container), landmarks (chapel, towers, ruins, gazebo, obelisk, fountain, market stall) and the **structure pieces** the RUN tool was written about — stone wall, fence, lamp post, pylon. A linear piece sets the SPACING slider to its own bay length when you pick it (a wall closes at 4 u, lamps pace at 16, pylons stride at 42); anything you dial afterwards still wins. Wall and fence bays lay their length along a RUN; houses and pylons face across it — a street fronts the street, wires hang across the pylons. |
