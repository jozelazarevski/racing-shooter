/* Reproduce the 110 km/h drift with telemetry: per-frame surface, lateral,
 * speed, to see where the energy goes. */
...
const r = await p.evaluate(() => {
  ...
  t.obstacles = []; t.solids = []; t.barriers = []; t.trees = []; if (t._width) t._width = new Float32Array(N).fill(30);
  ...
  for (let f = 0; f < 120; f++) {
    ...
    if ((f >= 65 && f <= 92) || f % 10 === 0 || f === 119) {
      rows.push({ f, v: +(Math.hypot(pl.vel.x, pl.vel.z) * 3.6).toFixed(0),
        lat: +(pl.lateral ?? 0).toFixed(1), i: pl.trackIndex,
        surf: pl.surface ?? pl._surf ?? null, off: pl.offRoad ?? null,
        y: +pl.pos.y.toFixed(2), air: pl.airborne ? 1 : 0, vy: +(pl.vy ?? 0).toFixed(2), wd: +(t.waterAt ? t.waterAt(pl.pos.x, pl.pos.z) : -1).toFixed(2), wet: +(pl._wetT ?? 0).toFixed(2),
        w: +t.widthAt(pl.trackIndex).toFixed(1) });
    }
  }
  return { stage, world: g.level?.name, rows };
});
