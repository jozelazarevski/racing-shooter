/* 6.5a balance-pass measurement — THE DAMAGE ECONOMY IN A REAL RACE.
 * Autopilot 3-lap races; per race: damage taken (by source class), regen
 * healed, minimum hull, wrecks. A street world and a rocky one. */
...
p.on('pageerror', () => {});
for (const [lvl, tag] of [[1, 'PINE'], [4, 'CANYON'], [3, 'FROST']]) {
  ...
  const res = await p.evaluate(async () => {
    ...
    const car = g.player;
    let dmgTotal = 0, dmgMax = 0, events = 0;
    const bySrc = {};
    const origDmg = car.damage.bind(car);
    car.damage = (amt, src) => {
      if (amt > 0.5) {
        dmgTotal += amt; events++;
        dmgMax = Math.max(dmgMax, amt);
        const k2 = src && g.enemies.includes(src) ? 'rival'
          : src && src.constructor?.name ? src.constructor.name : 'world';
        bySrc[k2] = (bySrc[k2] ?? 0) + amt;
      }
      return origDmg(amt, src);
    };
    let regen = 0, minHull = car.health, wrecks = 0, prevH = car.health;
    const skill = 0.94;
    let frames = 0;
    const CAP = 60 * 60 * 8;
    while (frames < CAP && !car.finished && !g.raceOver) {
      ...
      frames++;
      const h = car.health;
      if (h > prevH) regen += h - prevH;
      if (h < minHull) minHull = h;
      if (h <= 0 && prevH > 0) wrecks++;
      prevH = h;
    }
    return { world: g.level?.name, time: +g.raceTime.toFixed(0), rank: g.playerRank,
      maxHull: car.maxHealth, minHull: +minHull.toFixed(0),
      dmgTotal: +dmgTotal.toFixed(0), events, dmgMax: +dmgMax.toFixed(0),
      bySrc, regen: +regen.toFixed(0), wrecks };
  });
