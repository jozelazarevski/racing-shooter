...
for (const k of keys) {
  await p.evaluate((key) => {
    ...
    for (const k2 of ['engine','handling','tires','nitro','armor','cannon','dampers','magazine','rack']) up[k2] = 5;
    up.beacon = 3; up.copilot = 3;
    g.renderGarage();
    const st = g.__stage;
  ...
  if (el) await el.screenshot({ path: `${OUT}/carmax-${k}.png` });
}
await browser.close();
