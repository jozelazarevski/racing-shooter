...
console.log(' fov+ | camera.fov | bonnet    sky     ROAD');
for (const add of (process.env.FOVS ?? '6,12,18,24').split(',').map(Number)) {
  const r = await p.evaluate(async (add) => {
    ...
    g._driverTune = { up: 0.45, fwd: 0.14, lookH: 1.15, fov: add };
    ...
    pin(); await frame();
    const px = g1.w * g1.h;
    return { add, fov: +g.camera.fov.toFixed(1),
      body: +(body / px * 100).toFixed(1), sky: +(sky / px * 100).toFixed(1) };
  }, add);
  const road = +(100 - r.body - r.sky).toFixed(1);
  console.log(` ${String(r.add).padStart(4)} | ${String(r.fov).padStart(10)} | `
