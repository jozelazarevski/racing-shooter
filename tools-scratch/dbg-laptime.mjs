/* Lap-time probe: drives ONE lap with the airace expert stand-in at fixed
 * step and prints game-time lap seconds + lap length. Usage:
 *   LEVELS=2,20,68 node dbg-laptime.mjs
 */
...
for (const lvl of LEVELS) {
  ...
  const r = await p.evaluate(() => {
    ...
    const skill = 0.94;
    let lapT = null, frames = 0;
    const CAP = 60 * 60 * 12;   // 12 game-minutes hard cap
    while (frames < CAP && lapT === null) {
      ...
      g.frame();
      frames++;
      if ((car.lap ?? 1) >= 2) lapT = g.raceTime;
    }
    return { name: g.level?.name, lapT: lapT && +lapT.toFixed(1), lenU: Math.round(su * N),
      theme: g.level?.theme, capped: lapT === null };
  });
  console.log(`level ${lvl}  ${r.name} (${r.theme})  lap ${r.capped ? '>720 CAP' : r.lapT + ' s'}  length ${r.lenU} u`);
  await p.close();
}
await browser.close();
