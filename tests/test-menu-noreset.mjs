...
// --- track pick: scroll down, pick DUST CANYON, after reload tab/scroll restored
...
console.log('TRACK PICK:', JSON.stringify(rst), '(scroll restored if > 300)');
console.log('errors:', errors.slice(0, 3).join('|') || 'none');
const ok = before.url === after.url && after.key === 'sleek' && after.garageTab && after.driving && rst.lvl === 'DUST CANYON' && rst.scroll > 300;
