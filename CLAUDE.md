...
**Master spec (RALLY_MASTER_SPEC.md, 2026-09-07; REPLACED whole-document same day per its C-01 — v2 sections: HUD H-01..03, track T-01..05, containment W-EDGE-01 + W-CURVE-01 (standalone copy RALLY_W_CURVE_01.md), scenery S-01..05, vegetation V-01..02, validation VAL-01..03, change control C-01..02).** Supersedes RALLY_PATCH_02.md entirely; sits above this file in authority. One recorded conflict: the r364/r378 elevation mandates (passes 1200 m, terraces 500 m) are geometrically incompatible with WR-2.1 (grades 4–12%) plus §4 (lap target 5.5 km) — a lap climbing AND descending H at grade g needs length ≥ 2H/g, so 12% at 5.5 km caps the range near 330 m. The master spec is newer and higher authority: ELEV_MANDATE compresses to the grade-lawful range per world (min(mandate, lapLen·0.12/2·(1−a))), recorded here rather than silently traded. WR-8's altitude bands are relative to each stage's own max, so snow summits survive.

**Race Integrity patch (RALLY_RACE_INTEGRITY.md, 2026-09-09; owner-titled RALLY_PATCH_02, filed under its subject because RALLY_PATCH_02.md is the superseded 2026-09-06 document).** NORMATIVE, subordinate to RALLY_MASTER_SPEC.md; where it tightens a master rule the tighter one wins. Sourced from capture R21 (Falken Ridge, r407). IDs R-FINISH-01, W-EDGE-01a/02, R-RECOVER-01, C-CAM-01/02/03 (P0); G-DMG-01, G-WRECK-01, W-CURVE-02, G-AI-01, W-TEX-01 (P1); P-PERF-01 (P2).

TWO RECORDED CONFLICTS, raised rather than silently traded:
1. R-RECOVER-01's second trigger (off-road AND >15 m off the spline for 2 s -> auto-reset) reverses §3.6c (r345) and §3.6d (r364), which deleted exactly that class of return and kept only the physical traps. Its precedence clause defers to the MASTER SPEC, which does not cover these owner overrides. Triggers 1, 3 and 4 already meet the patch's own acceptance, so those ship and trigger 2 is held for the owner. Its 40 km/h rolling start also contradicts §3.6b (r324, `returnSpeedKmh` 0 — "a restarted car STANDS").
2. G-AI-01 bounds "rubber-banding" in both directions, but the rubber band was DELETED in r313 (§5 standing decision; `rubberBand`/`bandUp` are gone from EnemyCar, convergence lives only in the pressure rival's +-3% lease). There is no band to bound, so the observed pack-bunching and the 2nd-to-8th respawn swing have another cause and are to be root-caused, not papered over with a band.
...
**6.1b Race length, owner override (r364).** "I don't need 3 laps. Race is one lap only." The default lap count is 1 on every world; a world's own `laps` declaration still wins.

**6.1c Race length by track length, owner override (r381, 2026-09-06).** "Decide when track is 1 or 3 laps depending the length." A lap shorter than `lapsShortTrackU` (driving.json, 3000 u) races 3 laps; anything longer races 1 (the r364 one-lap rule becomes the long-track half). `laps` on a world still wins. Lap-shaped content is sized to the single lap: the CLEAN LAP contract ladder climbs by placement with a clean lap (rungs 2-3 were 2-3 clean laps and could never complete again), SURE-FOOTED asks for one clean lap, FLAWLESS START means leading across the line, and the final lap's time is recorded at the flag (it never was — the finish path returned before the best-lap write, on any lap count). Per-race score and credits shrink with the shorter race; recorded, not retuned, pending play.
...
| **7.4a Tunnels under mountains (owner, r350)** | "All Tunels needs to be under a mountain otherwise makes no sense." Every bore MUST lie under real terrain mass end to end (flank overburden >= 25 m along the whole bore, enforced in test-tunnels); the ridge line runs past both portals so approaches are cuttings into a face; on coast worlds the ridge stands as a headland over the sea depression; a station with another road leg in the ridge footprint takes no bore — no room for a mountain, no tunnel |
...
| **7.16 Sea view (owner, 2026-09-08)** | "Olive coast make it like small hills next to the sea." On a coast world the water MUST be visible from the coast road: the band between road and waterline is SMALL HILLS — rolling knolls on a shore-going envelope — never a mass that hides the bay (measured before: OLIVE COAST stood 80-92 m of terrain between road and sea and 0 of 55 coastal stations could see their own water, worst 117.7 m above the sightline). Acceptance is the sightline census, not the eye |
| **7.17 Elevation and climbs (owner, 2026-09-08)** | "Lower the height difference across the game. Instead of the straight climb introduce more interesting snake turns going up and down." Two binding parts: (a) the ROAD's height range per lap is modest — the surrounding mountains carry the drama, not the carriageway; (b) a sustained climb is SNAKED, never a straight ramp — it gains height through switchback/S turns AND undulates (rises and falls) on the way up, which is also master-spec T-04's "straight vertical-feeling ramps are prohibited". Hand-keyed `ascent` profiles are bound by both |
| **7.15 Forest containment (owner, 2026-09-08)** | "Car should not drive between the trees." A forest stand is a MASS, not a strainer: within the reachable verge belt, tree trunks stop a car like any solid (no 60%-and-through deflection), and the ground inside a dense stand (several trees within a car-length) carries heavy underbrush drag so threading the gaps is futile. Implemented as physics — visible trunks and brush, never an invisible wall (standing decision 1 stands). UNSTUCK remains the way out for a car wedged in the wood |
| **7.14 Evergreens (owner, 2026-09-08)** | "Pine trees are never not green. They are evergreen. Apply this across the game." Every pine-silhouette tree (pineA, pineB, fir — any stacked-cone/skirt conifer) renders green foliage on EVERY palette: autumn, harvest, moor, volcano, snow, dusk — no theme band, season, or altitude paint may tint a pine's needles off green. The r393 conifer tint law (sheet green #236555 divided through the canopy material, every theme) is the implement; every new tree system, imposter, or horizon painting that depicts conifers is bound by it. LARCH IS EXEMPT by the owner's own word ("Larch is getting yellow. All good", 2026-09-08, closing the census): it is the deciduous conifer and its autumn gold is the LARCH GOLD look. Broadleaf lobed crowns (maple, oak, birch, poplar) carry the autumn colours; a cold-light cast on green needles (FROST PEAK) is lighting, not a violation. Census verdict r398: pines/firs were ALREADY green on every palette — the law holds |
...
| HRD-2 | "This road needs widening" | Curvature widening: stations with corner radius < 45 u flare smoothly, up to +28% of base half-width at R <= 16 u (BOUNDED by W-CURVE-01.2: width through a sharp curve stays within +/-30% of approach width; the first cut used +55% and was brought inside the tolerance when W-CURVE-01 landed). Flare is additionally capped so the inner edge keeps >= 6 u of radius (no fold). It lands in `_buildWidthProfile` (the one profile every consumer reads) so mesh, AI clamp, fences, props and clearance rules follow together. Hairpin stations (R < 25) MUST carry the owed flare: min(1.19x base, max(base, R-6)) |
| HRD-3 | width floor | Drivable half-width never below 3.0 u anywhere, narrows included (measured: IL VICOLO's owner-directed lane sits at 3.04) |
| HRD-4 | width continuity | \|widthAt(i+1) - widthAt(i)\| <= 0.6 u — the road tapers, it never steps (measured: the steepest legitimate narrow shoulder is 0.32/station) |
| HRD-5 | "Add mountain on the one side. Apply across whole game when there is roads like this" + "there is always a mountain range from one of the sides of the road" (reference frame, 2026-09-07 20:31), SCOPED by the owner same evening: "there can be field roads. That is fine. This should apply in steep mountains" | On STEEP-MOUNTAIN terrain (and W-EDGE-01's narrow/canyon/gorge/valley classes), at every station at least one side MUST carry rising ground — a mountain flank/cut face within the corridor (the naturally higher side; landward on coast worlds; never flipping mid-run; trees/bushes/fences alone do NOT qualify, per W-EDGE-01.2). Field roads through flat country are exempt. Where the natural terrain already rises on a side, that side qualifies and nothing is built. Street segments satisfy this with their facades. Exclusions: start zone, bridges, gorge jumps, fords, plus W-EDGE-01.3's junction/overlook/shortcut gaps (<= 15 m each). The road-ceiling clamp still holds every leg clear. The owner's reference frame is the target look: faceted grey rock face with conifer clusters rising directly from the verge on one side, ground falling away on the other |
| HRD-6 | "No road floats or is on a ridge" | No station may have falling ground on BOTH sides (a causeway/ridge run), and no road edge stands unsupported in the air (3.8's conform rule, restated as a hard rule). A ridge-top route is re-flanked, not exempted |
| HRD-8 | "Rock too. Ad in rules md. No levitating objects" (2026-09-15, SALINE SPRINT frame: barrier blocks and a boulder standing clear of the ground) | NO LEVITATING OBJECTS. Every body meant to rest on the ground has its LOWEST WORLD-SPACE VERTEX within 0.15 m of `terrainHeight` at that vertex's own x,z — every class, every world, every leg. Measured from the lowest vertex, never the origin (K-21, and F-3's 1.8 m `structure-hover` under a green suite), and PER INSTANCE through each instance matrix (K-22). The gate WALKS THE SCENE GRAPH and fails closed: every mesh is either inside tolerance or on the named exemption list, and anything unclassifiable is a failure, not a skip — a registry the builders must be added to is why this class kept passing. Exempt: spans and decks between supports (piers and abutments are not), tunnel linings, cableways, the sea and anything floating on it (measured to the waterline), airborne effects, debris, birds, clouds, sky; pickups keep the tighter 7.12 rule. |
