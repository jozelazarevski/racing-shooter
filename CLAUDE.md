...
| HRD-5 | "Add mountain on the one side. Apply across whole game when there is roads like this" + "there is always a mountain range from one of the sides of the road" (reference frame, 2026-09-07 20:31) | AT EVERY STATION of every rural road, at least one side MUST carry rising ground — a mountain flank/cut face within the corridor (the naturally higher side; landward on coast worlds; never flipping mid-run). Where the natural terrain already rises on a side, that side qualifies and nothing is built. Street segments satisfy this with their facades. Exclusions: start zone, bridges, gorge jumps, fords. The road-ceiling clamp still holds every leg clear. The owner's reference frame is the target look: faceted grey rock face with conifer clusters rising directly from the verge on one side, ground falling away on the other |
